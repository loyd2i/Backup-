...
// POST - Upload de la pochette d'une track
export async function POST(
  ...
  try {
    ...
    const fileName = `${track.id}-cover-${Date.now()}-${Math.random().toString(36).slice(7)}.${ext}`;
    const buffer = Buffer.from(await file.arrayBuffer());
    await writeFile(path.join(uploadsDir, fileName), buffer);

    const url = `/uploads/tracks/${fileName}`;
    ...
    return NextResponse.json({ url, track: updated });
  } catch (error) {
    console.error('Erreur upload pochette track:', error);
